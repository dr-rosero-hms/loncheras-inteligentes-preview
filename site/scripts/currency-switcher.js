...
(() => {
  ...
  // ---------- Hotmart FX premium (por divisa) ----------
  // Hotmart cobra con margen adicional sobre la tasa de mercado (IOF, EBANX/dlocal,
  // fees de pasarela). Ajustamos para que el precio mostrado se acerque al
  // que verá el usuario al llegar al checkout, sin que la diferencia sorprenda.
  // Calibrado contra checkouts reales mayo 2026.
  const HOTMART_PREMIUM = {
    COP: 1.06,  // gap pequeño en Colombia
    MXN: 1.26,  // gap grande (México pasa por dlocal)
    ARS: 1.35,  // gap muy grande (peso muy volátil)
    CLP: 1.10,  // gap medio
    PEN: 1.08,  // gap pequeño
    EUR: 1.00,  // Hotmart suele cobrar EUR directo
    USD: 1.00,  // base
  };

  // Redondeo "psicológico" al alza por moneda
  const ROUND_UP_TO = {
    COP: 1000,    // $113.252 → $114.000
    MXN: 10,      // $649 → $650
    ARS: 1000,    // $32.450 → $33.000
    CLP: 100,     // $27.380 → $27.400
    PEN: 0.5,     // S/ 113.5 → S/ 114.0
    EUR: 0.5,     // €27.5 → €28.0
    USD: 1,       // $30 (no cambia)
  };
  ...
  function formatAmount(usdAmount, currency, rate) {
    ...
    const premium = HOTMART_PREMIUM[currency] || 1;
    const step = ROUND_UP_TO[currency] || 0;
    const raw = usdAmount * rate * premium;
    const local = roundUpToStep(raw, step);
  ...
  // ---------- Init ----------
  async function init() {
    // Esperar a price-sync ANTES del primer render, para usar el precio
    // actualizado desde Hotmart (con timeout interno de 1.2s en price-sync).
    if (window.__liPriceReady) {
      try { await window.__liPriceReady; } catch (_) {}
    }
    ...
    // Si price-sync actualiza el precio DESPUÉS del primer render (cache stale),
    // re-renderizar con el nuevo valor.
    window.addEventListener("li:price-updated", () => {
      applyCurrency(window.__liCurrency || finalCurrency, rates);
    });
  }
